...
#define BMP280_ADDRESS 0xEC  // SDIO is grounded, the 7 bit address is 0x76 and 8 bit address = 0x76<<1 = 0xEC
...
int BMP280_Config (uint8_t osrs_t, uint8_t osrs_p, uint8_t osrs_h, uint8_t mode, uint8_t t_sb, uint8_t filter)
{
	...
	if (HAL_I2C_Mem_Write(BMP280_I2C, BMP280_ADDRESS, RESET_REG, 1, &datatowrite, 1, HAL_MAX_DELAY) != HAL_OK)
	{
		check_bb = 10;
		return -1;
	}
	...
	if (HAL_I2C_Mem_Write(BMP280_I2C, BMP280_ADDRESS, CONFIG_REG_BMP280, 1, &datatowrite, 1, 1000) != HAL_OK)
	{
		return -4;
	}
	HAL_Delay (100);
	HAL_I2C_Mem_Read(BMP280_I2C, BMP280_ADDRESS, CONFIG_REG_BMP280, 1, &datacheck, 1, 1000);
	if (datacheck != datatowrite)
	{
		return -5;
	}
...
int BMPReadRaw(void)
{
	...
	if (chipID == 0x58)
	{
		...
		HAL_I2C_Mem_Read(&hi2c2, BMP280_ADDRESS, PRESS_MSB_REG, 1, RawData, 6, HAL_MAX_DELAY);
...
void BMP280_WakeUP(void)
{
	...
	HAL_I2C_Mem_Read(BMP280_I2C, BMP280_ADDRESS, CTRL_MEAS_REG, 1, &datatowrite, 1, 2000);

	// modify the data with the forced mode
	datatowrite = datatowrite | MODE_FORCED;

	// write the new data to the register
	HAL_I2C_Mem_Write(BMP280_I2C, BMP280_ADDRESS, CTRL_MEAS_REG, 1, &datatowrite, 1, HAL_MAX_DELAY);
